...
class Page {
  ...
  initDocker( options = {} ){
    return new Promise((resolve, reject) => {
      ...
      if (!options) return reject(new Error('No options supplied to initDocker'))
  ...
  initWebdriver( options = {} ){
    return new Promise((resolve, reject) => {
      ...
      // Allow for unit testing
      let promise = ( options.test !== undefined )
  ...
  testOpen(path){
    ...
    debug('test open %s', this_url)
    return needle('get', this_url).then(res => {
      debug('testOpen res', res.statusCode, res.body)
      return res
    })
  }
