...
class Page {
  ...
  constructor ( options = {} ){
    ...
    // Use selenium in a container (firefox/chrome)
    this.docker = Boolean(options.docker)
    let docker_host = (this.docker) ? Page.ip() : undefined
  ...
  initAsync(){
    ...
    this.promise = Promise.each(this.promises, i => i)
      .then(res => {
  ...
  initDocker(){
    return Docker.up(this.remote_browser)
      .then(res => {
        debug('initDocker docker is up - %s', this.remote_browser)
  ...
  initWebdriver( tryi = 0 ){
    return this.browser = webdriverio
      ...
      .catch(err => {
        if ( tryi < 10 ) {
          debug('initWebdriver trying selenium again', tryi)
          return Promise.delay(100).then(()=> this.initWebdriver(tryi+1))
        }
